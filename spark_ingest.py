...
query = hashtag_counts.selectExpr("topic", "CAST(topic AS STRING)", "CAST(value AS STRING)").writeStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092").option("checkpointLocation", "/tmp/dbt/checkpoint").start()
